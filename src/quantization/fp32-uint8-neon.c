...
#include <qnnpack/quantization-stubs.h>

void qnnp_quantize_fp32_uint8__neon(
    size_t n,
    const float* input,
    float scale,
    uint8_t zero_point,
...
#endif
  for (; n != 0; n -= 16) {
    const float32x4_t x = vld1q_f32(input);
    const float32x4_t y = vld1q_f32(input + 4);
    const float32x4_t z = vld1q_f32(input + 8);
    const float32x4_t w = vld1q_f32(input + 12);
    input += 16;
     ...
     */
    const float32x4_t x_scaled = vmulq_f32(x, vscale);
    const float32x4_t y_scaled = vmulq_f32(y, vscale);
    const float32x4_t z_scaled = vmulq_f32(z, vscale);
    const float32x4_t w_scaled = vmulq_f32(w, vscale);
