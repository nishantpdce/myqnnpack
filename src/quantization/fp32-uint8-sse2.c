...
void qnnp_quantize_fp32_uint8__sse2(
    size_t n,
    const float* input,
    float scale,
    uint8_t zero_point,
    ...
    uint8_t* output)
{
  ...
  const __m128 vscale = _mm_set1_ps(inv_scale);
  ...
  for (; n != 0; n -= 16) {
    const __m128 x = _mm_loadu_ps(input);
    const __m128 y = _mm_loadu_ps((input + 4));
    const __m128 z = _mm_loadu_ps((input + 8));
    const __m128 w = _mm_loadu_ps((input + 12));
    input += 16;

    /*
     * Multiply input by FP32 scale.
     ...
     */
    const __m128 x_scaled = _mm_mul_ps(x, vscale);
    const __m128 y_scaled = _mm_mul_ps(y, vscale);
    const __m128 z_scaled = _mm_mul_ps(z, vscale);
    const __m128 w_scaled = _mm_mul_ps(w, vscale);
